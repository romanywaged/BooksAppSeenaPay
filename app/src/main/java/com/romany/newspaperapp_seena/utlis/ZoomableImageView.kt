...
class ZoomableImageView(context: Context, attr: AttributeSet?) :
                ...
                if (mScaleFactor < 1) {
                    ...
                    if (mScaleFactor < 1) {
                        if (Math.round(origWidth * saveScale) < width) {
                            if (y < -bottom) newMatrix.postTranslate(
                                0f,
                                -(y + bottom)
                            ) else if (y > 0) newMatrix.postTranslate(0f, -y)
                        } else {
                            if (x < -right) newMatrix.postTranslate(
                                -(x + right),
                                0f
                            ) else if (x > 0) newMatrix.postTranslate(-x, 0f)
                        }
                    }
                }
            } else {
    ...
    init {
        ...
        mScaleDetector = ScaleGestureDetector(newContext, ScaleListener())
